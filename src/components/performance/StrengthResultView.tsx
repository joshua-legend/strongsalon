...
interface StrengthResultViewProps {
...
export default function StrengthResultView({
  category,
  result,
  prevResult,
  onConfirm,
  onRetry,
}: StrengthResultViewProps) {
  ...
  const prev1RM = prevResult?.estimated1RM ?? 0;
  ...
  return (
    <div className="px-4 py-6 flex flex-col min-h-[60vh]">
      ...
      <div className="flex flex-col items-center mb-6">
        ...
        <p className="font-mono text-sm" style={{ color: "var(--text-sub)" }}>추정 1RM: {result.estimated1RM.toFixed(1)} kg</p>
        <p className="font-mono text-sm" style={{ color: "var(--text-sub)" }}>체중 대비: {result.bodyweightRatio.toFixed(2)}배</p>
        <p className="font-mono text-sm" style={{ color: "var(--text-sub)" }}>점수: {result.score} / 100</p>
      </div>

      {prevResult && sameEquipment && prev1RM > 0 && (
        <div className="rounded-xl p-4 mb-6" style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border-light)" }}>
          ...
          <p className="text-sm" style={{ color: "var(--text-sub)" }}>{prevResult.estimated1RM.toFixed(1)}kg (1RM)</p>
          <p className={`text-sm font-bold mt-2 flex items-center gap-1 ${diff > 0 ? "text-lime-400" : diff < 0 ? "text-orange-400" : ""}`} style={diff === 0 ? { color: "var(--text-sub)" } : undefined}>
            ...
            {diff > 0 ? `+${diff.toFixed(1)}kg 향상!` : diff < 0 ? `${diff.toFixed(1)}kg` : "동일"}
          </p>
        </div>
      )}
