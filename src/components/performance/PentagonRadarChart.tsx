...
export default function PentagonRadarChart({ results }: PentagonRadarChartProps) {
  ...
  return (
    <div className="rounded-2xl p-5 bg-neutral-900 border border-neutral-800">
      ...
      <svg viewBox="0 0 300 300" className="w-full max-w-[280px] mx-auto block" style={{ minHeight: 280 }}>
        <defs>
        ...
        {gridPaths.map(({ level, path }) => (
          <path key={level} d={path} fill="none" stroke="#404040" strokeWidth="1" />
        ))}

        {axisLines.map(({ index, x2, y2 }) => (
          <line key={index} x1={cx} y1={cy} x2={x2} y2={y2} stroke="rgba(64,64,64,0.5)" strokeWidth="1" />
        ))}
        ...
        {vertices.map((v) => (
          <g key={v.index}>
            {v.isUnmeasured ? (
              <>
                ...
                <text x={v.outerX} y={v.outerY} textAnchor="middle" dominantBaseline="middle"
                  fill="#737373" fontSize="12" fontWeight="bold">?</text>
              </>
            ) : (
            ...
            <text x={v.labelX} y={v.labelY} textAnchor="middle" dominantBaseline="middle"
              fill="#a3a3a3" fontSize="10" fontFamily="ui-monospace, monospace">
              {v.label} {v.score ?? "?"}
            </text>
          </g>
        ))}
